...
async function route(req, res) {
  ...
  try {
    ...
    if (url.pathname === '/api/exec' && req.method === 'POST') { if (readOnly) return rejectReadOnly(res); const a=await body(req); const command=String(a.command||'').trim(); if (!validResourcePart(a.namespace) || !validResourcePart(a.name)) return send(res,400,{error:'Invalid namespace or pod name'}); if (!validExecCommand(command)) return send(res,400,{error:'Use one safe command without shell operators'}); if (process.env.NORTHSTAR_MODE !== 'kubernetes') return send(res,200,{output:`$ ${command}\n(simulated shell)\nNorthstar demo container is healthy.`}); const output=await execKubectl(a.context || context,['exec','-n',a.namespace,`pod/${a.name}`,'-c',a.container||'app','--','/bin/sh','-c',command]); return send(res,200,{output}); }
